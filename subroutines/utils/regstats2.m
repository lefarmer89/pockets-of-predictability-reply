function stats=regstats2(responses,data,model,whichstats) %#codegen
...
%----------------------------------------------------------------  
Y = y; x = X; 
allSeriesStats = cell2struct(num2cell(false(nnz(idx),1)),varnames(idx)); 
...
%----------------------------------------------------------------  
  ...
  if idx(27) % HC3
   ...
   if idx(29) % HAC (Newey West)
     ...
     L = ceil(0.75*(nobs^(1/3)));
     %L = 252;
     %L = round(nobs^.5); % as an alternative
     hhat = repmat(residuals',p,1).*X';
     xuux = hhat*hhat';
     for l = 1:L;
